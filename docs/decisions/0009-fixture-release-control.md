...
The public read contract is sufficient to drive that workflow without reading projections: `release show` and `release readiness` expose the complete definition, nullable canonical candidate and promotion digests and their bindings, and attestation evidence summaries with canonical attestation digests. `release list` remains summary-only.
