...
| TCP-10 | P1 | TCP-06 | Beamfall ticket mapping and import at scale: every shard field and status mapped or refused, provenance for completed tickets, 2501-ticket reads and writes measured against GP (decision 0011) | not-started; after Corvint 1.0 |
...
| TCP-12 | P1 | TCP-11 | Scheduling across one workspace's repositories: per-repository leases and merge locks, all-or-nothing `AtomicCrossRepo` tickets (decision 0011) | not-started; after Corvint 1.0 |
