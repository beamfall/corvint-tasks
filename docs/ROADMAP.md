...
The 2026-09-19 audit identified seven callable-writer defects. Their repair, named regressions
and remaining orchestration prerequisites are tracked in
[writer repair](reviews/2026-09-19-writer-repair.md). This work adds no admission or runtime.

Decision 0009 adds TM-V0-028 fixture release control and AS-38: bounded release projections, observed Git candidates, non-native manual/external attestations, derived readiness, local promotion, and durable replay. It adds no native gate runner, publication, tag, cutover, runtime, or real-queue authority.
...
| TCP-06 | P0 | TCP-01, TCP-03, TCP-05, TCP-21 | Batched import dry-run/apply, authority switch, roadmap projection, one real ticket in explicit-reviewer serial mode; GP rerun (TM-V0-023, 027; AS-26, AS-27, AS-31) | not-started; GP NOT_RUN |
...
| TCP-09 | P1 | TCP-00; operative use TCP-05, TCP-07; cutover TCP-10..14 | Beamfall adapter and recorder: Beamfall's roadmap moves to corvint-tasks after Corvint 1.0 (decision 0011); foreign execution disabled until qualified | not-started; owner Russell Lewis (decision 0011) |
| TCP-10 | P1 | TCP-06, TCP-19 | Beamfall ticket mapping and import at scale: every shard field and status mapped or refused, provenance for completed tickets, 2501-ticket reads and writes measured against GP (decision 0011) | not-started; after Corvint 1.0 |
| TCP-11 | P1 | TCP-05 | Integration step: rebase, gate on the result, fast-forward merge under a per-repository lock, completion recorded in the store (decision 0011) | not-started; after Corvint 1.0 |
| TCP-12 | P1 | TCP-11, TCP-20 | Scheduling across one workspace's repositories: per-repository leases and merge locks, all-or-nothing `AtomicCrossRepo` tickets (decision 0011) | not-started; after Corvint 1.0 |
| TCP-13 | P1 | TCP-10 | Generated Beamfall shard projection that `roadmap_lib.parse` accepts, so Beamfall readers work unchanged through cutover (decision 0011) | not-started; after Corvint 1.0 |
| TCP-14 | P1 | TCP-02 | Per-stage claim, review and merge authority as policy (Beamfall ADR-0204 D6) (decision 0011) | not-started; after Corvint 1.0 |
| TCP-15 | P1 | none | Scale benchmark harness and measured baseline: deterministic Beamfall-calibrated generator, datasets S-2.5k/10k/100k (history R=N and 10N), B-30 and E-1k, fresh-process timing with p50/p95/p99 and bootstrap intervals, cold cache, syscall and bytes-read counts, 5/10/20-writer simulators (decision 0012) | not-started; quiet host only |
| TCP-16 | P1 | TCP-15 | Read-path quick wins with no format change: one tree capture and hash per attempt instead of three, Tarjan only for verbs that need it (decision 0012) | not-started |
| TCP-17 | P1 | TCP-15 | `taskman-index/0` derived immutable index: builder, `index verify`/`index rebuild`, index-mode reads from journal-canonical state, text search, differential and fault-injection tests (decision 0012 A1–A4) | not-started; needs owner acceptance of A2 and of A4's reader-lease point |
| TCP-18 | P1 | TCP-17 | Incremental writer: O(1) request replay, touched-record checks through the index, incremental capacity accounting, lock fairness measured before it is built (decision 0012 A5) | not-started; needs owner acceptance of A5 |
| TCP-19 | P1 | TCP-18, TCP-21, TCP-02 (B4) | `taskman-state/1` migration: 100k tickets, `tickets/<xx>/` fan-out, sharded import map, saturation accounting, checkpoint and prune (decision 0012 A6) | not-started; needs owner acceptance of A6 |
| TCP-20 | P1 | TCP-10, TCP-17 | Beamfall-sized workspace: repository attribute and postings, membership records, cross-repository reads within budget, federation-ready keys and roots (decision 0012 A7, A8) | not-started |
| TCP-21 | P1 | TCP-02 | Per-transaction artifact cap raised to at least 210 so a 100-record `IMPORT_APPLY` batch commits; lands inside TCP-19 if older binaries cannot read such a transaction (decision 0012) | not-started; prerequisite of TCP-06 |
...
`receipt audit` was wired on 2026-09-19 as a pure full-journal structural audit; all unobserved
acceptance/runtime axes remain explicit. Not implemented: `receipt show|replay`, `config show`, `attempt show`,
`plan preview` (they need journal, pinned or plan data that no TCP-01 reader can observe) and
remaining administrative mutation verbs; each still answers `NOT_RUN`. The fourteen
`ticket` mutation verbs were wired by TCP-02b on 2026-09-07 and now commit; remaining administrative
verbs still answer `NOT_RUN` after the delivered fixture reconciliation/pause/unpause increments. §3.3 mutation
...
plans and never commits; TCP-02b subsequently wired the ticket CLI, with no runtime qualification claimed. Attempt, receipt replay and planner surfaces are later slices.
